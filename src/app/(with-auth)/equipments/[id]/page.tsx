...
import { adminEditEquipmentAction, userEditEquipmentAction } from "@/action/edit-equipment.action";
...
export default function NewEquipmentPage() {
  ...
  const { data, isLoading } = useQuery<Equipments>({
    ...
    queryFn: async () => {
      ...
      if(response.status === 200 || response.status == 304) {
        setUf(ufs?.find((uf) => uf.sigla === response.data.uf)?.id ?? null);
        form.setValue("name", response.data.nome);
        form.setValue("description", response.data.descricao);
        form.setValue("mac", response.data.mac);
        form.setValue("consumerUnityNumber", response.data.unidade_consumidora.numero);
        form.setValue("subGroup", response.data.subgrupo);
        form.setValue("uf", response.data.uf);
        form.setValue("city", response.data.cidade);
        form.setValue("monitoredPhases", response.data.fases_monitoradas as "MONOFASE" | "BIFASE" | "TRIFASE");
        form.setValue("ratedVoltage", response.data.tensao_nominal.toString());
      }
  ...
  const form = useForm<NewEquipmentSchemaType>({
    defaultValues: {
      ...
      uf: data?.uf,
      city: data?.cidade,
      monitoredPhases: data?.fases_monitoradas as "MONOFASE" | "BIFASE" | "TRIFASE",
  ...
  async function onSubmit(values: NewEquipmentSchemaType) {
    ...
    if(user?.perfil === Role.ADMIN){
      response = await adminEditEquipmentAction(values, params.id.toString()).finally(() => {
        setLoading(false);
      });
    }else{
      response = await userEditEquipmentAction(values, params.id.toString()).finally(() => {
        setLoading(false);
      });
    }
                      ...
                      <FormItem className="max-w-40">
                        <FormControl>
                          <Input
                            ...
                            disabled={loading || !isAdmin || isLoading}
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                      ...
                      <FormItem className="max-w-40 w-full">
                        <FormControl>
                          <Select
                            ...
                            defaultValue={field.value}
                            disabled={ufLoading || loading || !isAdmin || isLoading}
                          >
                            <FormControl>
                              <SelectTrigger
                                label="UF"
